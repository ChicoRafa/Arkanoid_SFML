...
public:
    ...
    void removeTileAt(const sf::Vector2f& position)
    {
        // Convert the position to tile coordinates
        int tileX = static_cast<int>(position.x / m_mapTileSize.x);
        int tileY = static_cast<int>(position.y / m_mapTileSize.y);

        // Check if the coordinates are within bounds
        if (tileX >= 0 && tileX < m_chunkCount.x * m_chunkSize.x / m_mapTileSize.x &&
            tileY >= 0 && tileY < m_chunkCount.y * m_chunkSize.y / m_mapTileSize.y)
        {
            // Get the chunk and the relative position within the chunk
            sf::Vector2u chunkLocale;
            auto& chunk = getChunkAndTransform(tileX, tileY, chunkLocale);

            // Remove the tile by setting its ID to 0 (assuming 0 is an empty tile)
            chunk->setTile(chunkLocale.x, chunkLocale.y, tmx::TileLayer::Tile{0}, true);
        }
    }
...
public:
    ...
    //We cannot let this be constant, as we need to do some modifications later on
